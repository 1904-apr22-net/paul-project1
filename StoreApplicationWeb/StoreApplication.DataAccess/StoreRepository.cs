   ...
   public class StoreRepository : Library.Interfaces.IStoreRepository
    {
        ...
        public IEnumerable<Customer> GetNames(string search = null)
        {
            ...
            if (search != null)
            {
                items = items.Where(r => (r.Fname + " " + r.Lname).Contains(search));
            }
            return Mapper.Map(items);
        }
        public IEnumerable<Customer> GetCustomerAtID(int ID = -1)
        {
            ...
            if (ID != -1)
            {
                items = items.Where(r => r.ConsumerId == ID);
            }
            return Mapper.Map(items);
        }
        public IEnumerable<Location> GetLocationAtId(int ID = -1)
        {
            _logger.Info($"Retrieving Orders");
            ...
            if (ID != -1)
            {
                items = items.Where(r => r.StoreId == ID);
            }
            return Mapper.Map(items);
        }
        /// <summary>
        ...
        public IEnumerable<Order> GetOrdersList(List<Customer> customer = null)
        {
            _logger.Info($"Retrieving Orders");
            ...
            if (customer != null)
            {
                List<Orders> itemsL = new List<Orders>();
                foreach(var c in customer)
                {
                    itemsL.Add(items.Where(r => r.ConsumerId == c.CustomerId).ToList().FirstOrDefault());
                }
                items = itemsL.AsQueryable();
                
            }
            return Mapper.Map(items);
        }
        public IEnumerable<Order> GetOrdersL(Location customer = null)
        {
            _logger.Info($"Retrieving Orders");
            ...
            if (customer != null)
            {
                items = items.Where(r => r.StoreId == customer.LocationId);
            }
            return Mapper.Map(items);
        }
        public IEnumerable<Order> GetOrdersAtId(int ID = -1)
        {
            _logger.Info($"Retrieving Orders");
            ...
            if (ID != -1)
            {
                items = items.Where(r => r.OrderId == ID);
            }
            return Mapper.Map(items);
        }
        public IEnumerable<OrderDetails> GetOrderDetailsAtId(int ID = -1)
        {
            _logger.Info($"Retrieving Orders");

            IQueryable<OrderItem> items = _dbContext.OrderItem
                .Include((x) => x.Order)
                .Include(x => x.Product);
        ...
        public int GetProductAmount(Product product, Order order)
        {
            _logger.Info($"Getting Recommended Products");
            ...
            items = items.Where(r => r.ProductId == product.ProductId && r.OrderId == order.OrderId);
            var mapped = Mapper.Map(items).ToList().FirstOrDefault();
        ...
        /// </summary>
        /// <returns></returns>
        public IEnumerable<Product> DisplayProducts(Order order = null)
        {
            ...
            if (order != null)
            {
                var items2 = new List<Products>();
                foreach(var i in items)
                {
                    foreach(var o in order.OrderDetails)
                    {
                        if(o.ProductId == i.ProductId)
                        {
                            items2.Add(i);
                        }
                    }
                }
                return Mapper.Map(items2);
            }
            return Mapper.Map(items);
        }
